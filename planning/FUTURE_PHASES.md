...
- Some events conflict and should not appear at the same time (e.g. signal interference / signal breakthrough)
...
- Signal mini-games and especially at the end with the Wormhole - see `planning/SIGNAL_MINIGAME.md`
- Progress seems to jump from 33% to about 65% when the first signal tech is unlocked.
- The signal shouldn't be from 'beyond the heliopause' - it should be within the solar system, probably the Kuiper belt.
...
- For how long should the player be allowed to stay in debt? A lose condition? But it's tough to recover from a debt spiral.
...
- Sometimes I've been able to reach the wormhole and skip over the 66% signal threshold
